...
public class EmbeddedMongoDBServer {
    ...
    private String databaseName = "prices-crawler-data";
    ...
    public EmbeddedMongoDBServer start() {
        if (!this.active && !inUse(this.port)) {
            try {
                ...
                this.mongoClient = MongoClients.create(String.format("mongodb://%s:%s", host, port));
            } catch (final IOException e) {
                log.error("EmbeddedMongoDBServer failed to start - {}:{}", this.host, this.port, e);
            }
        } else {
            log.error("EmbeddedMongoDBServer failed to start.");
        }

        return this;
    }

    public void addCollectionFromJson(String collection, String json) {
        mongoClient.getDatabase(databaseName).getCollection(collection).insertOne(Document.parse(json));
    }
    ...
    private boolean inUse(int port) {
        try (var serverSocket = new ServerSocket(port, 0, InetAddress.getByName(host))) {
            ...
            log.error("EmbeddedMongoDBServer - port is already in use - {}:{}", this.host, this.port, e);
            return true;
        }
    }
}